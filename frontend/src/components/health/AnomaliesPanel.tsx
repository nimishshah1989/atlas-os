...
const SEV_STYLE: Record<NonNullable<AnomalyRow['severity']>, string> = {
  critical: 'border-l-sig-neg bg-sig-neg-soft text-sig-neg',
  warn: 'border-l-sig-warn bg-sig-warn/10 text-sig-warn',
  info: 'border-l-brand bg-brand-soft text-brand',
}
...
export function AnomaliesPanel({ anomalies }: { anomalies: AnomalyRow[] }) {
  return (
    <div className="px-6 py-5 border-b border-edge-hair">
      ...
      {anomalies.length === 0 ? (
        <div className="font-sans text-sm text-sig-pos">
            ...
            return (
              ...
              >
                <div className="flex items-baseline justify-between gap-4">
                  ...
                  <div className="font-num text-[12px] tabular-nums">
                    ...
                    {a.z_score !== null && (
                      <span className="ml-2 text-txt-3">{formatZ(a.z_score)}</span>
                    )}
                  </div>
                </div>
                {a.notes && (
                  <div className="font-sans text-[11px] text-txt-3 mt-1">{a.notes}</div>
                )}
              </li>
            )
          })}
