...
type OptColKey =
  | 'ret_1w' | 'ret_6m' | 'ret_12m' | 'ret_12m_1m'
  | 'rs_pctile_1m_vt' | 'rs_pctile_6m_vt'
  | 'rs_pctile_3m_acwi' | 'rs_pctile_3m_eem' | 'rs_pctile_3m_gold'
  | 'consensus' | 'above_30w_ma' | 'ema_10_ratio' | 'ema_20_ratio'
  | 'extension_pct' | 'realized_vol_63' | 'max_drawdown_252' | 'atr_21'
...
const OPT_COLS: { key: OptColKey; label: string }[] = [
  ...
  { key: 'ret_12m_1m',            label: 'RS Mom' },
  { key: 'rs_pctile_1m_vt',      label: 'RS 1M' },
  { key: 'rs_pctile_6m_vt',      label: 'RS 6M' },
  { key: 'rs_pctile_3m_acwi',    label: 'RS ACWI' },
  { key: 'rs_pctile_3m_eem',     label: 'RS EEM' },
  { key: 'rs_pctile_3m_gold',    label: 'RS Gold' },
  { key: 'consensus',             label: 'Consensus' },
  { key: 'above_30w_ma',          label: '30W MA' },
  { key: 'ema_10_ratio',          label: 'EMA10 Ratio' },
  { key: 'ema_20_ratio',          label: 'EMA20 Ratio' },
  ...
  { key: 'stage1_base_qualifies', label: 'Stage1 Base' },
]
const LS_KEY = 'us-stock-screener-cols'
const DEFAULT_OPT_COLS: OptColKey[] = ['ret_1w', 'ret_6m', 'ret_12m', 'above_30w_ma', 'atr_21']
...
type SortCol =
  | 'ticker' | 'gics_sector' | 'rs_state' | 'momentum_state'
...
type QuickFilter = 'all' | 'sp500' | 'leader_strong' | 'above_30w' | 'investable' | 'accel_improving'
const QUICK_FILTERS: { key: QuickFilter; label: string }[] = [
  { key: 'all',             label: 'All' },
  { key: 'sp500',           label: 'S&P 500' },
  { key: 'leader_strong',   label: 'Leader/Strong' },
  { key: 'above_30w',       label: 'Above 30W' },
...
export function USStockScreener({ stocks }: { stocks: USStockRow[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (quickFilter === 'leader_strong')   rows = rows.filter(r => r.rs_state === 'Leader' || r.rs_state === 'Strong')
    if (quickFilter === 'above_30w')       rows = rows.filter(r => r.above_30w_ma === true)
    if (quickFilter === 'investable')      rows = rows.filter(r => r.history_gate_pass === true && r.liquidity_gate_pass === true)
    if (quickFilter === 'accel_improving') rows = rows.filter(r => r.momentum_state === 'Accelerating' || r.momentum_state === 'Improving')
    return [...rows].sort((a, b) => {
      const dir = sortDir === 'asc' ? 1 : -1
      if (sortCol === 'ticker')       return dir * a.ticker.localeCompare(b.ticker)
      if (sortCol === 'gics_sector')  return dir * (a.gics_sector ?? '').localeCompare(b.gics_sector ?? '')
      ...
      type G = (r: USStockRow) => string | null
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="h-4 w-px bg-paper-rule" />
        <select
          value={sector}
        ...
        <div className="relative" ref={colsRef}>
          ...
          {colsOpen && (
            <div className="absolute top-full mt-1 left-0 z-30 w-56 bg-paper border border-paper-rule rounded shadow-lg p-3">
        ...
        <span className="ml-auto font-sans text-xs text-ink-tertiary">{filtered.length} stocks</span>
      </div>
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="border-b border-paper-rule sticky top-0 bg-paper z-10">
              ...
              <th className="px-3 py-2 font-sans text-[10px] font-semibold uppercase tracking-wider text-ink-tertiary whitespace-nowrap">Vol</th>
              ...
              {isVisible('ret_12m_1m')         && <Th label="RS Mom"   col="ret_12m_1m"        align="right" />}
              {isVisible('rs_pctile_1m_vt')    && <Th label="RS 1M"   col="rs_pctile_1m_vt"   align="right" />}
              {isVisible('rs_pctile_6m_vt')    && <Th label="RS 6M"   col="rs_pctile_6m_vt"   align="right" />}
              {isVisible('rs_pctile_3m_acwi')  && <Th label="RS ACWI" col="rs_pctile_3m_acwi" align="right" />}
              {isVisible('rs_pctile_3m_eem')   && <Th label="RS EEM"  col="rs_pctile_3m_eem"  align="right" />}
              {isVisible('rs_pctile_3m_gold')  && <Th label="RS Gold" col="rs_pctile_3m_gold" align="right" />}
              {isVisible('consensus') && (
                <th className="px-3 py-2 font-sans text-[10px] font-semibold uppercase tracking-wider text-ink-tertiary whitespace-nowrap text-right">Consensus</th>
              )}
              ...
              {isVisible('effort_ratio_63')    && <Th label="Effort"  col="effort_ratio_63"    align="right" />}
              ...
              {isVisible('stage1_base_qualifies') && (
                <th className="px-3 py-2 font-sans text-[10px] font-semibold uppercase tracking-wider text-ink-tertiary whitespace-nowrap text-right">Stage1</th>
              )}
            </tr>
          </thead>
          <tbody>
              ...
              pageRows.map((row, i) => (
                ...
                >
                  ...
                  <td className="px-3 py-2.5 max-w-[110px]">
                    <span className="font-sans text-xs text-ink-secondary truncate block" title={row.gics_sector ?? ''}>
                      {row.gics_sector ?? '—'}
                    </span>
                  </td>
                  ...
                  <td className="px-3 py-2.5"><StateChip value={row.volume_state}   colors={VOL_STATE_COLORS}  /></td>
                  ...
                  {isVisible('ret_12m_1m') && <td className={`px-3 py-2.5 text-right font-mono text-xs tabular-nums ${fmtPctColor(row.ret_12m_1m)}`}>{fmtPct(row.ret_12m_1m)}</td>}
                  ...
                  {isVisible('rs_pctile_6m_vt')   && <td className="px-3 py-2.5 text-right font-mono text-xs tabular-nums text-ink-secondary">{fmtPctile(row.rs_pctile_6m_vt)}</td>}
                  ...
                  {isVisible('rs_pctile_3m_eem')  && <td className="px-3 py-2.5 text-right font-mono text-xs tabular-nums text-ink-secondary">{fmtPctile(row.rs_pctile_3m_eem)}</td>}
                  ...
                  {isVisible('consensus') && (
                    <td className="px-3 py-2.5 text-right font-mono text-xs tabular-nums">
                      {row.rs_consensus_bullish != null || row.rs_consensus_bearish != null ? (
                        <span>
                          <span className="text-signal-pos">{row.rs_consensus_bullish ?? 0}{'↑'}</span>
                          {' '}
                          <span className="text-signal-neg">{row.rs_consensus_bearish ?? 0}{'↓'}</span>
                        </span>
                      ) : '—'}
                    </td>
                  )}
                  ...
                  {isVisible('effort_ratio_63')   && <td className="px-3 py-2.5 text-right font-mono text-xs tabular-nums text-ink-secondary">{fmtRatio(row.effort_ratio_63, '', 2)}</td>}
                  {isVisible('weinstein_gate_pass') && (
                    <td className="px-3 py-2.5 text-right">
                  ...
                  {isVisible('stage1_base_qualifies') && (
                    <td className="px-3 py-2.5 text-right">
                      <span className={`font-mono text-xs ${row.stage1_base_qualifies === true ? 'text-signal-pos' : row.stage1_base_qualifies === false ? 'text-signal-neg' : 'text-ink-tertiary'}`}>
                        {fmtBool(row.stage1_base_qualifies)}
                      </span>
                    </td>
                  )}
                </tr>
