...
const OPTIONAL_COLS: ColumnDef[] = [
  ...
  // Phase 8: within-state rank from atlas_etf_signal_unified.mean_within_state_rank.
  // Additive alongside rs_pctile_3m (returns-based). NULL gracefully until
  // Agent A's aggregator populates mean_within_state_rank.
          ...
          <thead>
            <tr className="border-b border-paper-rule bg-paper">
              ...
              {visibleCols.has('within_state_rank') && (
                <PlainTh label="Within Rank" align="right" title="Within-state rank [0..1] from atlas_etf_signal_unified. Additive alongside RS Pctile (returns-based). NULL = Agent A aggregator pending." />
              )}
                    ...
                    >
                      ...
                      {visibleCols.has('within_state_rank') && (
                        <td className="px-3 py-2.5 text-right" data-testid={`etf-wsr-${row.ticker}`}>
                          {/* TODO: depends on Agent A's etf aggregator view rewrite */}
