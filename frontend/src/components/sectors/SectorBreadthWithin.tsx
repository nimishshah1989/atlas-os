...
export function SectorBreadthWithin({ stocks }: { stocks: SectorStock[] }) {
  ...
  // lead is the 0/1 leader flag: 1 = top-decile composite within the stock's cap cohort.
  const leadDist = [1, 0].map(l => ({ l, c: stocks.filter(s => s.lead === l).length }))
  ...
  return (
    <section className="px-8 py-10 border-b border-edge-hair" aria-label="Within-sector breadth">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-10 gap-y-6">
        ...
        <div>
          <div className="font-num text-[10px] text-txt-3 uppercase tracking-wider mb-2">Leadership distribution (top-decile composite within cap cohort)<TermInfo term="leadership_dist" /></div>
          <div className="space-y-1.5">
            {leadDist.map(({ l, c }) => (
              <div key={l} className="flex items-center gap-3">
                <span className="w-[86px] shrink-0 font-num text-xs tabular-nums text-txt-2">{l >= 1 ? 'Leader' : 'Not a leader'}</span>
                ...
                <span className="flex-1 h-[7px] bg-surface-inset rounded-tile overflow-hidden">
                  <span className={`block h-full rounded-tile ${l >= 1 ? 'bg-sig-pos' : 'bg-txt-3/40'}`}
