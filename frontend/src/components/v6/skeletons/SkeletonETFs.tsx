// frontend/src/components/v6/skeletons/SkeletonETFs.tsx
//
// Page layout schema (design-application.md §6.5 — ETFs mirror Funds layout):
//   4 top-level children:
//     1. header strip     — eyebrow + H1
//     2. industry snapshot — 4 callout cards (no AMC leaderboard for ETFs)
//     3. charts section   — bubble chart + signature matrix
//     4. ranked table     — with column chooser
//
// ETF list page. Layout mirrors SkeletonFunds; categories differ.

import { Shimmer } from './Shimmer'

interface SkeletonETFsProps {
  className?: string
}

export function SkeletonETFs({ className = '' }: SkeletonETFsProps) {
  return (
    <div data-testid="page-root" className={`min-h-screen bg-[#F8F4EC] p-6 space-y-6 ${className}`}>
      ...
      <div className="space-y-2">
        ...
        <Shimmer width="w-60" height="h-4" />
      </div>

      {/* 2 — industry snapshot cards */}
