// frontend/src/components/v6/skeletons/SkeletonETFDetail.tsx
//
// Page layout schema (design-application.md §6.6 — ETF detail mirrors Fund detail):
//   3 top-level children:
//     1. header strip — ETF name + grade chip + conviction tape + thesis
...
interface SkeletonETFDetailProps {
  className?: string
}

export function SkeletonETFDetail({ className = '' }: SkeletonETFDetailProps) {
  return (
      ...
      <div className="space-y-3 pb-4 border-b border-[#C2B8A8]">
        ...
        <div className="space-y-2">
          {Array.from({ length: 3 }, (_, i) => (
            <Shimmer key={i} width={i === 1 ? 'w-5/6' : 'w-full'} height="h-4" />
          ))}
        </div>
      </div>
