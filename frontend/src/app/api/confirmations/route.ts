// GET /api/confirmations?code=alpha&week=YYYY-MM-DD — one week of the FM's
...
import { getConfirmation, getOpeningBook, listConfirmations } from '@/lib/queries/confirmations'

export const dynamic = 'force-dynamic'

const isCode = (v: string | null): v is PortfolioCode =>
  v != null && (PORTFOLIO_CODES as readonly string[]).includes(v)
...
export async function GET(req: Request) {
  ...
  if (!isCode(code)) {
    return NextResponse.json(
      { error_code: 'bad_request', message: `code must be one of ${PORTFOLIO_CODES.join(', ')}` },
      { status: 400 },
    )
  }
