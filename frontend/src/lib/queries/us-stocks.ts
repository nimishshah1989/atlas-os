...
export type USStockRow = {
  ...
  // RS vs VT (primary benchmark) — 5 timeframes × pctile + raw
  ...
  // RS consensus (bullish = # of 20 cells above median; bearish = # below)
...
export type USSectorBreadthRow = {
  gics_sector: string
  rs_state: string
  cnt: number
}

export async function getUSStocks(): Promise<USStockRow[]> {
  return sql<USStockRow[]>`
    WITH latest AS (
    ...
    LEFT JOIN us_atlas.atlas_stock_states_daily s
...
export async function getUSSectorBreadth(): Promise<USSectorBreadthRow[]> {
  return sql<USSectorBreadthRow[]>`
    WITH latest_date AS (
    ...
    WHERE s.date = ld.d
      ...
      AND s.rs_state IS NOT NULL
    GROUP BY s.gics_sector, s.rs_state
    ORDER BY s.gics_sector, s.rs_state
  `
}
