...
export const GLOSSARY: Record<string, GlossaryEntry> = {
  ...
  rank_trend: { title: 'Rank trend', body: 'The fund’s category rank over time — one thin slice per trading day, green (best in category) → red (worst); hover a slice for its date and rank. The rank moves daily because the holdings are re-scored against each day’s stock lens scores. “stable Xd” = days held at the current rank; “swing a/b” = the 30-day and 90-day max−min rank range (small = steady, large = churny). History runs from ~Feb 2026, when holdings first became available.' },
