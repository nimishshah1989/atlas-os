...
export const GLOSSARY: Record<string, GlossaryEntry> = {
  ...
  leadership_dist: { title: 'Leadership distribution', body: 'How many of the sector’s constituents are leaders — in the TOP DECILE (D10) of composite score within their cap cohort — vs not.' },
  ff_weight: { title: 'Free-float weight in sector', body: 'The stock’s share of the SECTOR’s total FREE-FLOAT market cap (free-float = market cap × the non-promoter, non-ESOP shareholding). A concentration read: a few names near 100% = top-heavy; weights spread evenly = dispersed. NOTE: this is free-float weight within Atlas’s own sector constituents — live and UN-capped — NOT the published NSE index weight. It can differ a lot from an index factsheet because (a) our sector set differs from the index (e.g. our Defence excludes Bharat Forge / Solar Industries, which we file under Auto / Chemicals), (b) we apply no single-stock cap, and (c) it’s live, not rebalance-dated. Free-float is why a govt-heavy name weighs LESS than its size suggests: BEL and HAL have near-identical market cap, but the Government holds 72% of HAL vs 51% of BEL, so BEL’s free float (and weight) is ~1.7× HAL’s. Stocks without market-cap or shareholding data show "—".' },
  ...
  leadership_breadth: { title: 'Leadership breadth', body: 'Share of holdings (by weight) that are leaders — in the TOP DECILE (D10) of composite score within their cap cohort. Breadth shows whether strength is broad or concentrated.' },
  ...
  lead: { title: 'Leader', body: 'A stock is a LEADER when it sits in the TOP DECILE (D10) of composite score within its cap cohort (large/mid/small/micro) — one simple rule. Shown as Leader / —.' },
  ...
  leaders_count: { title: 'Leaders', body: 'Holdings that are leaders — in the TOP DECILE (D10) of composite score within their cap cohort. The base for leadership-breadth.' },
  ...
  fund_returns: { title: 'Absolute return (NAV)', body: 'The fund’s own NAV total return over 1W / 1M / 3M / 6M / 12M — the raw return, not relative to any benchmark. Green = positive, red = negative. Shown next to the relative-strength cell so you see both the raw performance and how it compares to the market. As of the latest published NAV.' },
  ...
  golden_cross: { title: '# Golden cross', body: 'How many holdings are in a GOLDEN CROSS — 50-day EMA above their 200-day EMA, the classic mark of a long-term uptrend. Shown as golden / priced. This is the dominant driver of the score: Technical is ~90% of the composite and is built on EMA structure (EMA50>EMA200 → +10, EMA21>EMA50 → +15), so a fund whose book is mostly in golden crosses scores high.' },
