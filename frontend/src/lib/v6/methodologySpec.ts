...
// its roll-ups to sector / ETF / fund — not the older −10..+10 conviction copy on /methodology.
...
export const METHODOLOGY: MethoNode[] = [
  {
    ...
    children: [
      ...
      },
      {
        ...
        plain: 'Is the business actually good — profitable, growing, not over-leveraged? Sum of four sub-components.',
        formula: 'Fundamental score = Profitability + Margin + Growth + Balance sheet (points)',
        children: [
      ...
      },
      {
        ...
        plain: 'Is informed money accumulating the stock? A weighted average of ownership and delivery signals.',
        children: [
          { id: 'flow_prom', title: 'Promoter', plain: 'Promoter (founder) ownership — rising stake is a positive signal.', children: [m('promoter')] },
          { id: 'flow_smart', title: 'Smart money', plain: 'Institutional / informed accumulation.', children: [m('smart_money')] },
          { id: 'flow_acc', title: 'Accumulation (delivery)', plain: 'Real ownership change, not intraday churn.', children: [m('delivery'), m('delivery_asym')] },
        ],
      },
      {
        id: 'catalyst', title: 'Catalyst', weight: '0.20',
        plain: 'Are there real events — order wins, results, capital actions — moving the stock? A weighted read of exchange filings.',
        children: [
          { id: 'cat_earn', title: 'Earnings & momentum', plain: 'Results and guidance filings.' },
          { id: 'cat_cap', title: 'Capital actions', plain: 'Buybacks, raises, M&A, order wins.' },
          { id: 'cat_gov', title: 'Governance', plain: 'Disclosure / governance filings — lower weight, context only.' },
        ],
      },
      {
  ...
  },
  {
    ...
    plain: 'Beyond the holdings roll-up, each fund gets a 0–100 scorecard and is ranked against the other funds in its SEBI category.',
    formula: 'Fund score = risk-adjusted return + holdings conviction + style/sector fit + cost. Category rank = position by score within the funds shown.',
    children: [
      m('fund_score'),
      m('cat_rank'),
      m('expense'),
