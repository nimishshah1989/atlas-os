...
/** Generic response shape for portfolio API calls. */
export type ApiResponse<T = unknown> =
  | { ok: true; data: T; status: number }
  | { ok: false; error_code: string; message: string; status: number }

/**
 * Generic helper for calling the internal FastAPI backend.
 * Handles auth header, JSON body, and error envelope parsing.
 */
export async function callInternalApi<T = unknown>(
  ...
  let res: Response
  try {
    res = await fetch(`${INTERNAL_BASE_URL}${path}`, {
      method,
      headers: {
  ...
  if (res.ok) {
    return { ok: true, data: (parsed?.data ?? parsed) as T, status: res.status }
  }

  return {
    ok: false,
    error_code: parsed?.detail?.error_code ?? parsed?.error_code ?? 'api_error',
    message: parsed?.detail?.message ?? parsed?.message ?? `HTTP ${res.status}`,
    status: res.status,
  }
}
