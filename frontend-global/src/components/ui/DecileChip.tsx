...
// so the table reads in greyscale, to a colour-blind reader, and in a screenshot pasted into a deck.
//
// The chip names its step as the `--decile` custom property and the stylesheet does the drawing:
// ink on a 30% tint of it, with the full step as the border. Setting `background` to the ramp
// colour directly would put dark ink on a saturated ground — the tint is what keeps all ten steps
// legible, and its contrast was checked across the whole ramp when the palette landed.
...
export function DecileChip({
  ...
  return (
    <span
      ...
      style={{ '--decile': colour } as CSSProperties}
...
export function LeaderMark({ decile }: { decile: number | null | undefined }) {
  ...
  return (
    <span
      ...
      title={`Top decile (${LEADER_DECILE} of 10) within its peer group`}
    >
      Leader
    </span>
  )
