...
describe('DecileChip', () => {
  it('paints each decile with its own ramp token and prints the number inside it', () => {
    for (let n = 1; n <= 10; n += 1) {
      ...
      // The chip names its ramp step as a custom property; globals.css draws ink on a 30%
      // tint of it. Asserting `backgroundColor` would pin the chip to a saturated ground,
      // which is the version whose contrast fails on the middle of the ramp.
      expect(chip?.style.getPropertyValue('--decile')).toBe(`var(--decile-${n})`)
