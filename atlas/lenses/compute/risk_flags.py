...
def _net_insider_selling(signals: list[dict[str, Any]], th: dict[str, Any]) -> dict[str, Any] | None:
    net = Decimal(0)
    for s in signals:
    ...
    if net > Decimal(str(th.get("insider_net_sell_cr", 0.5))):
        return {"name": "net_insider_selling", "penalty": th.get("penalty_net_insider_selling", -8),
...
def _pledge_increasing(signals: list[dict[str, Any]], th: dict[str, Any]) -> dict[str, Any] | None:
    for s in signals:
        if (s.get("type") or "").lower() == "pledge_increase":
            return {"name": "pledge_increasing", "penalty": th.get("penalty_pledge_increasing", -6),
                    "description": "Promoter pledge increasing"}
    return None


def _multiple_insiders_selling(signals: list[dict[str, Any]], th: dict[str, Any]) -> dict[str, Any] | None:
    sellers = {s.get("name") for s in signals
               if (s.get("type") or "").lower() == "sell" and s.get("name")}
    if len(sellers) >= th.get("insider_min_distinct_sellers", 3):
        return {"name": "multiple_insiders_selling",
...
def _margin_declining(margins: list[float | None], th: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if consec >= min_consec:
        return {"name": "margin_declining", "penalty": th.get("penalty_margin_declining", -6),
...
def _revenue_declining(fin: dict[str, Any], th: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if rev is None or rev_prev is None or rev_prev <= 0:
        return None
    if rev < rev_prev:
        return {"name": "revenue_declining", "penalty": th.get("penalty_revenue_declining", -5),
...
def _leverage_up_margins_down(fin: dict[str, Any], th: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if de > de_prev and em < em_prev:
        return {"name": "leverage_up_margins_down",
...
def _check_filings(
    filings: list[dict[str, Any]], th: dict[str, Any],
) -> list[dict[str, Any]]:
    ...
    for f in filings:
        ...
        if not found_aud and any(kw in subj for kw in auditor_kw):
            results.append({"name": "auditor_change",
        ...
        if not found_res and any(r in subj for r in roles) and any(w in subj for w in resign_kw):
            results.append({"name": "cfo_ceo_resignation",
        ...
        if not found_crd and "credit rating" in subj and "downgrade" in subj:
            results.append({"name": "credit_downgrade",
...
def _price_below_200dma(price: float | None, ema_200: float | None, th: dict[str, Any]) -> dict[str, Any] | None:
    if price is None or ema_200 is None or ema_200 <= 0:
        return None
    if price < ema_200 * th.get("price_below_200dma_pct", 0.85):
        return {"name": "price_below_200dma", "penalty": th.get("penalty_price_below_200dma", -3),
