...
**Follow-up in the same session — data foundation widened so the real books fit**

Seeding the FM's three real books exposed two gaps (both now closed, see
`docs/adr/0004-coverage-750-and-etf-sectors.md`):

1. **Coverage 500 → 750.** `JSFB` and `LLOYDSENGG` (8% each in Alpha) were
   `is_active=false` — not stale, just outside the Nifty 500 coverage universe. Coverage
   is now NIFTY 500 ∪ NIFTY MICROCAP250 (NSE's Nifty Total Market; disjoint, so exactly
   750). Applied: 498 → **747** active stocks, 0 deactivated. Gates still pass — 0
   missing sectors, 21 distinct, 0 non-canonical. `build_universe.py` maintains it, so
   the nightly run will not revert it. **Scored universe is ~50% larger — watch the
   nightly compute window.**
2. **Every ETF now has a sector.** All 318 were NULL and Passive is 100% ETFs. New
   `scripts/foundation/etf_sector.py` derives it from the ETF name: canonical-21 where
   the ETF tracks a sector index, else one of exactly five asset classes (Gold, Silver,
   Debt, Broad Index, International — FM decision). 318 written, 0 NULL. **Re-run
   `etf_sector.backfill()` after any ETF listing sync — it is NOT yet in atlas_daily.sh.**

**All three books are seeded and verified** (week of 20-Jul-2026, so 27-Jul is free for
real calls). Each stored book reproduces the FM's sheet exactly, 0% unmapped:
Alpha 12 positions / 99% invested, Passive 18 / 99.5%, India XI 11 / 99%.
Three Passive tickers were ambiguous on the sheet and confirmed by the FM:
Smallcap=`SMALLCAP` (Mirae momentum-quality), Mid150etf=`MID150` (Kotak),
Chemicals=`CHEMICAL` (Kotak).

Seed data deliberately lives only in the DB, never in the repo — atlas-os is a **public**
GitHub repo and these are live positions.
...
3. ~~Seed the three books~~ — DONE, see above.
