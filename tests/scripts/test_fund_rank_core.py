...
class TestComposite:
    ...
    def test_returns_none_when_no_weighted_lens_present(self):
        ...
        assert C.composite({"v_tech": None, "v_fund": 40.0, "v_flow": None, "v_cat": 9.0}, W_2LENS) is None
        assert C.composite({"v_tech": None, "v_fund": None, "v_flow": None, "v_cat": None}, W_DEFAULT) is None


class TestRankInCategory:
    def _mk(self, mstar_id, v, weights, breadth=0.0, category="India Fund Multi-Cap"):
        return {"mstar_id": mstar_id, "category": category, "breadth": breadth,
                "composite": C.composite(v, weights)}

    def test_ranks_by_composite_desc_with_size_over_scored_cohort(self):
        rows = [self._mk("boi", BANK_OF_INDIA, W_DEFAULT),
    ...
    def test_ties_broken_by_breadth_then_mstar_id(self):
        rows = [self._mk("low", HSBC, W_DEFAULT, breadth=0.1),
    ...
    def test_identical_everything_breaks_by_mstar_id_unique_ranks(self):
        rows = [self._mk("bbb", HSBC, W_DEFAULT, breadth=0.5),
    ...
    def test_separate_categories_rank_independently(self):
        rows = [self._mk("a", GROWW, W_DEFAULT, category="Large-Cap"),
    ...
    def test_unscored_fund_has_no_rank_but_counts_scored_size(self):
        rows = [self._mk("hsbc", HSBC, W_DEFAULT),
                {"mstar_id": "x", "category": "India Fund Multi-Cap", "breadth": 0.0, "composite": None}]
        out = {r["mstar_id"]: r for r in C.rank_in_category(rows)}
...
class TestPctBand:
    @pytest.mark.parametrize("rank,size,band", [
        (1, 1, "Top 10%"),
        (1, 50, "Top 10%"),
