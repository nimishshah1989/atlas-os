...
# allow-large: comprehensive unit tests covering all 6 endpoints with edge cases —
# splitting into multiple files would fragment the shared mock helpers and fixture
...
def _make_leader_row(
    ...
    return_since_open: float | None = 0.0156,
...
def _mock_engine_with_rows(rows: list[tuple]) -> MagicMock:
    ...
    fetchone() returns None so secondary queries (e.g. nifty return lookup)
    don't produce invalid Decimal conversions.
    """
    mock_conn = MagicMock()
...
class TestIndicesEndpoint:
    ...
    def test_indices_response_shape_has_required_fields(self, client: TestClient) -> None:
        """Each item has symbol, bar_time, open, high, low, close, return_since_open, pct_change."""
    ...
    def test_indices_symbol_values_present(self, client: TestClient) -> None:
        """symbol field matches the values written by the ingester."""
        mock_engine = _mock_engine_with_rows(_SAMPLE_INDICES_ROWS)
        with patch("atlas.api.intraday.get_engine", return_value=mock_engine):
            response = client.get("/api/v1/intraday/indices")
    ...
    def test_indices_null_return_since_open_handled(self, client: TestClient) -> None:
        """NULL return_since_open comes back as null, pct_change also null — no crash."""
        null_row = _SAMPLE_INDICES_ROWS[2]  # NIFTY IT row with None return
    ...
    def test_indices_empty_table_returns_empty_list_with_note(self, client: TestClient) -> None:
        """Empty table returns data=[], not an error, with explanatory note."""
        mock_engine = _mock_engine_with_rows([])
        with patch("atlas.api.intraday.get_engine", return_value=mock_engine):
            response = client.get("/api/v1/intraday/indices")
        assert response.status_code == 200
    ...
    def test_indices_meta_data_as_of_present_when_data_available(self, client: TestClient) -> None:
        """meta.data_as_of is set to bar_time ISO string when data is present."""
        mock_engine = _mock_engine_with_rows(_SAMPLE_INDICES_ROWS)
        with patch("atlas.api.intraday.get_engine", return_value=mock_engine):
            response = client.get("/api/v1/intraday/indices")
        meta = response.json()["meta"]
    ...
    def test_indices_close_price_is_decimal_not_float(self, client: TestClient) -> None:
        """close prices come back as Decimal-compatible strings (no floating point)."""
        mock_engine = _mock_engine_with_rows(_SAMPLE_INDICES_ROWS[:1])
        with patch("atlas.api.intraday.get_engine", return_value=mock_engine):
            response = client.get("/api/v1/intraday/indices")
        item = response.json()["data"][0]
        # Pydantic serialises Decimal as string in JSON; must parse to Decimal without error
        price = Decimal(str(item["close"]))
        assert price == Decimal("24530.00")


# ---------------------------------------------------------------------------
# INDEX_TOKENS constant (rs_engine)
# ---------------------------------------------------------------------------


class TestIndexTokens:
    def test_index_tokens_contains_nifty50(self) -> None:
        """INDEX_TOKENS includes the canonical NIFTY 50 token."""
    ...
    def test_index_tokens_correct_token_values(self) -> None:
        """Kite instrument tokens match the spec values."""
        from atlas.intraday.rs_engine import INDEX_TOKENS
