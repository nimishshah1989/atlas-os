...
def run() -> None:
    ...
    strmap = dict(zip(j["instrument_id"], j["strength"], strict=False))
    snap = _db.scalar("SELECT max(as_of_date) FROM atlas_foundation.de_mf_holdings")

    h = _db.read_df(
        """SELECT m.fund_name, m.category_name, m.amc_name, h.instrument_id, h.weight_pct
           FROM atlas_foundation.de_mf_holdings h JOIN atlas_foundation.de_mf_master m USING (mstar_id)
    ...
    print("\n-- TOP 20 by leadership-breadth --")
    print(hdr)
    print("-" * len(hdr))
    for _, r in f.head(20).iterrows():
        print(
            f"{str(r['fund_name'])[:40]:40s} {str(r['category_name'])[:16]:16s} "
            f"{int(r['n']):>4d} {r['breadth_%']:>6.1f} {r['strength']:>4.1f}"
        )
    print("\n-- BOTTOM 8 by leadership-breadth --")
    print(hdr)
    print("-" * len(hdr))
    ...
    for _, r in cat.iterrows():
        print(
            f"{str(r['category_name'])[:22]:22s} {int(r['funds']):>5d} {r['breadth']:>8.1f} {r['strength']:>4.1f}"
        )
