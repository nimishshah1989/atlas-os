...
def run(limit: int | None = None) -> dict:
    ...
    iid_map = dict(zip(df["symbol"], df["instrument_id"].astype(str)))
    if limit:
    ...
    from atlas.tv.screener import _fetch_tv_batch, _upsert_rows, _label, _json_dumps
    ...
    for i in range(0, len(symbols), batch_size):
        ...
        for rec in records:
            ...
            rows.append({
                ...
                "ema_20": rec.get("EMA20"), "ema_50": rec.get("EMA50"),
                "ema_200": rec.get("EMA200"), "atr_14": rec.get("ATR"),
                "volume": int(rec["volume"]) if bool(pd.notna(vol)) else None,
                "volume_10d_avg": int(rec["average_volume_10d_calc"]) if bool(pd.notna(vol10d)) else None,
                "price": rec.get("close"),
                "high_52w": rec.get("High.All"), "low_52w": rec.get("Low.All"),
                ...
                "stoch_k": rec.get("Stoch.K"), "stoch_d": rec.get("Stoch.D"),
                "adx": rec.get("ADX"), "adx_plus_di": rec.get("ADX+DI"),
                "adx_minus_di": rec.get("ADX-DI"),
                "cci_20": rec.get("CCI20"), "williams_r": rec.get("W.R"),
                "mfi": rec.get("MFI"), "ao": rec.get("AO"), "uo": rec.get("UO"),
                "momentum": rec.get("Mom"), "roc": rec.get("ROC"),
                "bb_lower": rec.get("BB.lower"), "bb_upper": rec.get("BB.upper"),
                "vwap": rec.get("VWAP"),
                "sma_20": rec.get("SMA20"), "sma_50": rec.get("SMA50"),
                "sma_200": rec.get("SMA200"),
                "perf_1w": rec.get("Perf.W"), "perf_1m": rec.get("Perf.1M"),
                "perf_3m": rec.get("Perf.3M"), "perf_6m": rec.get("Perf.6M"),
                "perf_ytd": rec.get("Perf.YTD"), "perf_1y": rec.get("Perf.Y"),
                ...
                "avg_volume_30d": int(rec["average_volume_30d_calc"]) if bool(pd.notna(rec.get("average_volume_30d_calc"))) else None,
                "avg_volume_60d": int(rec["average_volume_60d_calc"]) if bool(pd.notna(rec.get("average_volume_60d_calc"))) else None,
        ...
        if (i // batch_size) % 5 == 0 or i + batch_size >= len(symbols):
            print(f"[tv_metrics] {min(i+batch_size, len(symbols))}/{len(symbols)} upserted={total}", flush=True)
