...
from harness import (BENCHMARKS, CAL_INDEX, COMPLETENESS_MIN, JUMP_MAX_PCT,
                     STALE_MAX_TDAYS, STAGING_SCHEMA, GREEN, RED, YEL, DIM, RST)
...
def calendar() -> pd.DatetimeIndex:
    df = _db.read_df(f"select date from {M}.index_prices where index_code=:c order by date",
                     {"c": CAL_INDEX})
...
def universe(asset_class: str) -> pd.DataFrame:
    df = _db.read_df(
        ...
        "where asset_class=:a and kite_token is not null order by symbol", {"a": asset_class})
...
def _covcl(asset_class, uni, cal, demergers) -> dict:
    ...
    by_key_sym = dict(zip(uni["instrument_id"] if keycol == "instrument_id" else uni["symbol"],
                          uni["symbol"]))
    ...
    for i in range(0, len(keys), 200):
        ...
        for k, g in df.groupby("k"):
            ...
            agg[k] = {"first": c.index.min(), "last": c.index.max(), "n": len(c),
    ...
    for r in uni.itertuples():
        ...
        a = agg.get(str(k))
        if not a:
            res[r.instrument_id] = {"symbol": r.symbol,
        ...
        cal_in_range = cal[(cal >= span_start) & (cal <= a["last"])] if len(cal) else pd.DatetimeIndex([])
        ...
        res[r.instrument_id] = {
            "symbol": r.symbol,
            "coverage": {"pass": bool(cov_ok), "rows": a["n"], "first": str(a["first"].date()),
                         "depth": round(completeness, 4),
                         "reason": "ok" if cov_ok else f"rows={a['n']} depth={completeness:.1%}"},
            "cleanliness": {"pass": not cl, "maxjump": round(a["maxjump"], 4),
...
def _metrics_parity(asset_class, uni) -> dict:
    ...
    px = _db.read_df(
        ...
        "where instrument_id = any(cast(:ids as uuid[])) group by 1", {"ids": ids})
    tcount = dict(zip(px["id"].astype(str), px["n"]))
    ...
    if keycol == "instrument_id":
        oc = _db.read_df(f"select cast(instrument_id as text) id, count(*) n from {tbl} "
                         f"where instrument_id = any(cast(:ids as uuid[])) {valid_filter} group by 1",
                         {"ids": ids})
        ocount = dict(zip(oc["id"].astype(str), oc["n"]))
        key_to_iid = {i: i for i in ids}
    else:
        syms = list(uni["symbol"])
        oc = _db.read_df(f"select {keycol} k, count(*) n from {tbl} where {keycol}=any(:ks) "
                         f"{valid_filter} group by 1", {"ks": syms})
        sym_n = dict(zip(oc["k"].astype(str), oc["n"]))
    ...
    for r in uni.itertuples():
        ...
        res[r.instrument_id] = {"pass": bool(ok), "ohlcv": o, "tech": t,
...
def run(asset_class: str, sample: int | None = None) -> dict:
    ...
    for r in uni.itertuples():
        ...
        rows.append({"instrument_id": r.instrument_id, "symbol": r.symbol,
                     "coverage": c.get("coverage", {}), "cleanliness": c.get("cleanliness", {}),
    ...
    green = sum(1 for x in rows if x["coverage"].get("pass") and x["cleanliness"].get("pass")
                and x["metrics"].get("pass"))
    summary = {"asset_class": asset_class, "n": n, "coverage_pass": cov,
               "cleanliness_pass": cl, "metrics_pass": mt, "green": green}
...
def _report(ac, rows, s):
    ...
    for axis in ("coverage", "cleanliness", "metrics"):
        ...
        if fails:
            print(f"  {DIM}{axis} fails ({len(fails)}): "
                  + ", ".join(f"{r['symbol']}[{r[axis].get('reason','')}]" for r in fails[:6])
                  + (" …" if len(fails) > 6 else "") + RST)
