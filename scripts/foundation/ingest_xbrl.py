...
def list_quarters(s: requests.Session, symbol: str) -> list[dict]:
    r = s.get(_API, params={"index": "equities", "symbol": symbol, "period": "Quarterly"}, timeout=30)
    r.raise_for_status()
    return r.json()
...
def parse_xbrl(xml: bytes) -> dict:
    ...
    for el in root.iter():
        name = ln(el.tag)
        if name in _TAGS and el.get("contextRef") == _QTR_CTX and el.text and el.text.strip():
...
def ingest_symbol(s: requests.Session, iid: str, symbol: str) -> int:
    recs = list_quarters(s, symbol)
    # keep latest revision per (period, consolidated): highest seqNumber
    best: dict = {}
    for r in recs:
        if r.get("period") != "Quarterly" or r.get("cumulative") != "Non-cumulative":
            continue
    ...
    rows = []
    for (pe, consol), (seq, url, is_bank) in best.items():
        try:
            xml = requests.get(url, headers=_H, timeout=30).content
            fin = parse_xbrl(xml)
        except Exception:
            continue
        time.sleep(0.25)
        if fin.get("revenue") is None and fin.get("pat") is None:
            continue
        rows.append({"instrument_id": iid, "symbol": symbol, "period_end": pe,
                     "consolidated": consol, **fin, "is_bank": is_bank,
                     "seq_number": seq, "xbrl_url": url})
    if not rows:
        return 0
    return _db.upsert_df(f"{M}.financials_quarterly", pd.DataFrame(rows),
...
def run(only_pending=True, limit=None) -> dict:
    ...
    total = len(tgt); done = err = 0; qtot = 0
    s = session()
    print(f"[xbrl] targets={total}", flush=True)
    for n, r in enumerate(tgt.itertuples(), 1):
        iid, sym = r.instrument_id, r.symbol
        try:
            q = ingest_symbol(s, iid, sym)
            _db.upsert_df(f"{M}.xbrl_state", pd.DataFrame([{
                "instrument_id": iid, "symbol": sym, "status": "done" if q else "no_data",
                "quarters": q, "error": None, "updated_at": dt.datetime.now(dt.UTC)}]),
                ["instrument_id"]); done += 1; qtot += q
        ...
        if n % 25 == 0 or n == total:
            print(f"[xbrl] {n}/{total} done={done} err={err} quarters={qtot} last={sym}", flush=True)
        time.sleep(0.5)
    print(f"[xbrl] COMPLETE done={done} err={err} quarters={qtot}", flush=True)
