...
def _composite(row, th) -> float:
    ...
    return float(compute_composite(
        technical=f(row["technical"]), fundamental=f(row["fundamental"]),
        valuation_score=f(row["valuation"]), catalyst=f(row["catalyst"]),
        flow=f(row["flow"]), policy=f(row["policy"]),
        valuation_multiplier=1.0, smart_money_score=0.0, degradation_score=0.0,
        thresholds=th).final_score)
...
def _sector_index_map() -> dict:
    ...
    return dict(zip(m["sector_name"], m["primary_nse_index"]))
...
def run() -> None:
    ...
    px = _db.read_df(f"SELECT index_code, date, close FROM {IDX} "
                     "WHERE index_code = ANY(:c) AND close>0",
                     {"c": idx_codes})
    px["date"] = pd.to_datetime(px["date"])
    panel = px.pivot_table(index="date", columns="index_code", values="close").reindex(sessions)

    print(f"sectors={sld['sector'].nunique()} dates={sld['date'].nunique()} "
    ...
    for h in HORIZONS:
        ...
        print(f"  h={h:>3} sessions: cross-sectional sector-rotation IC = {mean_ic:+.4f} "
