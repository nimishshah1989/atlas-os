...
def _rs_table() -> dict:
    ...
    idxmap = dict(_db.read_df(
        f"SELECT sector_name, primary_nse_index FROM {SM} "
        f"WHERE is_active AND primary_nse_index IS NOT NULL").values)
    codes = tuple(sorted(set(list(idxmap.values()) + [BENCH])))
    px = _db.read_df(f"SELECT index_code, date, close FROM {IDX} "
                     f"WHERE index_code IN :c AND close>0 ORDER BY date", {"c": codes})
    ...
    for sec, code in idxmap.items():
        if code not in ser:
            continue
        out[sec] = {lbl: (None if (_trailing(ser[code], w) is None or bret.get(lbl) is None)
                          else _trailing(ser[code], w) - bret[lbl]) for lbl, w in WINDOWS}
    return out
...
def run() -> None:
    ...
    sec = g.agg(n=("instrument_id", "size"),
                leaders=("lead2", "sum"),
    ...
    hdr = (f"{'sector':18s} {'n':>4s} {'lead':>4s} {'brdth%':>6s} {'str':>4s} | "
           + " ".join(f"{lbl:>6s}" for lbl, _ in WINDOWS))
    print(hdr); print("-" * len(hdr))
    for _, r in sec.iterrows():
        rscells = " ".join(f"{rcell(r['sector'], lbl):>6s}" for lbl, _ in WINDOWS)
        print(f"{str(r['sector'])[:18]:18s} {int(r['n']):>4d} {int(r['leaders']):>4d} "
              f"{r['breadth_%']:>6.1f} {r['strength']:>4.1f} | {rscells}")
    print("  breadth% = share of constituents top-decile in >=2 conviction lenses (within cap cohort, OUR view)")
    print(f"  1m/3m/6m/1y = sector index trailing return minus {BENCH}'s, in % (OBSERVED, not a forecast)")
