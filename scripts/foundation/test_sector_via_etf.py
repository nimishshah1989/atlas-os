...
def run() -> None:
    ...
    hold = _db.read_df(
        """SELECT ticker, instrument_id, weight FROM public.de_etf_holdings
    ...
    t2s = dict(zip(etfs["ticker"], etfs["sector_name"], strict=False))
    s2idx = dict(zip(etfs["sector_name"], etfs["primary_nse_index"], strict=False))
    hold["sector"] = hold["ticker"].map(t2s)

    # atom sub-scores
    j = _db.read_df(
        f"SELECT instrument_id, date, {','.join(SUBS)} "
    ...
    sess = list(
        pd.to_datetime(
            _db.read_df(
                f"SELECT DISTINCT date FROM {IDX} WHERE index_code='NIFTY 50' ORDER BY date"
            )["date"]
        )
    )
    ...
    for h in HORIZONS:
        ...
        for dt, grp in sld.groupby("date"):
            pts = [
                (r["composite"], fwd.at[dt, s2idx[r["sector"]]])
                for _, r in grp.iterrows()
                if s2idx.get(r["sector"]) in fwd.columns
                and dt in fwd.index
        ...
        print(
            f"  h={h:>3}: ETF-based sector-rotation IC = {np.mean(ics):+.4f} (over {len(ics)} dates)"
        )
