...
def targets(asset_classes: list[str] | None, only_pending: bool, limit: int | None) -> pd.DataFrame:
    q = (
        f"select instrument_id, asset_class, symbol, isin, kite_token "
...
def run(asset_classes=None, only_pending=True, limit=None, end=None) -> dict:
    ...
    total = len(tgt)
    print(
        f"[backfill] targets={total} classes={asset_classes or 'all'} "
        f"depth>={HIST_START} end={end}",
        flush=True,
    )
    ...
    for n, r in enumerate(tgt.itertuples(), 1):
        ...
        try:
            ...
            if df.empty:
                ...
                _mark(iid, ac, sym, "done", rows=w, df=df)
                done += 1
                rows_tot += w
        except Exception as e:
            msg = repr(e)
            _mark(iid, ac, sym, "error", err=msg)
            err += 1
            if any(
                t in msg
                for t in ("TokenException", "Insufficient permission", "api_key", "access_token")
            ):
                print(
                    f"[backfill] AUTH FAILURE at {sym}: {msg[:160]} — stopping; "
                    "re-auth and re-run to resume.",
                    flush=True,
                )
                break
        if n % 25 == 0 or n == total:
            print(
                f"[backfill] {n}/{total}  done={done} nodata={nodata} err={err} "
