...
def main():
    ...
    print(
        "== Data-integrity gate (freshness · real sub-scores · breadth EMA21 · returns · mapping · taxonomy) =="
    )
    ...
    g.check(
        "technicals fresh (≤4 days old)",
        days_old(tech_d) <= 4,
        f"technical_daily={tech_d}, today={today}",
    )
    g.check(
        "lens journal current with technicals", lens_d == tech_d, f"lens={lens_d} tech={tech_d}"
    )
    ...
    for col, floor in [
        ("fund_profitability", 50),
        ("val_pe_vs_sector", 50),
        ("flow_institutional", 50),
    ]:
        nd = _q(
            f"select count(distinct {col}) from {LD} where asset_class='stock' and date=:d",
            {"d": lens_d},
        )
    ...
    pol_hits = _q(
        f"select count(*) from {LD} where asset_class='stock' and date=:d and policy_tailwind > 0",
        {"d": lens_d},
    )
    g.check(
        "policy alerts produced (real sector matches)",
        (pol_hits or 0) >= 20,
        f"{pol_hits} stocks flagged by a policy",
    )
    ...
    g.check(
        ...
        has21,
        "pct_above_ema21 present" if has21 else "still pct_above_ema20",
    )
    ...
    if mvb_d:
        ...
        g.check(
            "breadth EMA21 populated (latest)", bool(tot) and nn21 >= 0.95 * tot, f"{nn21}/{tot}"
        )
        g.check(
            "breadth EMA200 populated (latest)", bool(tot) and nn200 >= 0.95 * tot, f"{nn200}/{tot}"
        )
    ...
    if mvc_d:
        ...
        bad = _q(
            f"select count(*) from {MVC} where as_of_date=:d and (abs(ret_6m)>1.0 or abs(ret_3m)>1.0)",
            {"d": mvc_d},
        )
        g.check(
            "sector returns in sane range (|3m|,|6m|≤100%)", bad == 0, f"{bad} sectors out of range"
        )
        null12 = _q(
            f"select count(*) from {MVC} where as_of_date=:d and ret_12m is null", {"d": mvc_d}
        )
        g.check("ret_12m populated for all sectors", null12 == 0, f"{null12} null ret_12m")

    # 5 — SECTOR MAPPING complete (the 368 unmapped)
    unmapped = _q(
        f"select count(*) from {IM} where asset_class='stock' and is_active and (sector is null or sector='')"
    )
    ...
    has_rollup = bool(
        _q("select count(*) from information_schema.tables where table_name='atlas_sector_rollup'")
    )
    g.check("sector rollup table exists (D13 fold)", has_rollup, "atlas_sector_rollup")
    nsec = _q(
        f"select count(distinct sector) from {IM} where asset_class='stock' and sector is not null and sector<>''"
    )
