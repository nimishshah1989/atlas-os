...
def targets(only_pending: bool, limit):
    ...
    if only_pending:
            ...
            f"select instrument_id from {M}.lens_shareholding_state where status='done'"
        )
...
def run(only_pending=True, limit=None) -> dict:
    ...
    for n, r in enumerate(tgt.itertuples(), 1):
        ...
        if n % 25 == 0 or n == total:
            print(
                f"[shareholding] {n}/{total} done={done} err={err} qtrs={qtot} last={sym}",
                flush=True,
            )
        time.sleep(1.0)
