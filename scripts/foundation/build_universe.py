...
_ETF_MARKERS = re.compile(
    ...
    r"HEALTHCARE|MIDCAP|SMALLCAP|INFRA|DIVOPP|CPSE|BHARAT22)", re.I)
...
def fetch_equity_list() -> pd.DataFrame:
    ...
    out = pd.DataFrame({
        ...
        "listing_date": pd.to_datetime(df["DATE OF LISTING"], format="%d-%b-%Y",
...
def build() -> dict:
    ...
    cash_tok = {i["tradingsymbol"]: int(i["instrument_token"])
    ...
    for sym, tok in cash_tok.items():
        ...
        if _ETF_MARKERS.search(sym) or _ETF_MARKERS.search(nm):
            rows.append((uuid_for("etf", sym), "etf", sym, cash_name.get(sym), None, None,
                         None, tok, "NSE", True, "KITE_NSE_ETF"))
    # indices
    for i in idx:
        sym = i["tradingsymbol"]
        rows.append((uuid_for("index", sym), "index", sym, i.get("name"), None, None,
                     None, int(i["instrument_token"]), "NSE", True, "KITE_INDICES"))

    cols = ["instrument_id", "asset_class", "symbol", "name", "isin", "series",
            "listing_date", "kite_token", "exchange", "is_active", "source"]
    ...
    counts = df.groupby("asset_class").agg(
        total=("symbol", "size"),
        on_kite=("kite_token", lambda s: int(s.notna().sum()))).to_dict("index")
