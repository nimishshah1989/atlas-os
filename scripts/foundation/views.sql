...
--
-- Refresh: `refresh materialized view concurrently atlas_foundation.<name>;`
...
drop materialized view if exists atlas_foundation.mv_breadth_nifty500_daily;
create materialized view atlas_foundation.mv_breadth_nifty500_daily as
select t.date,
       count(*)                                          as n_total,
...
create unique index if not exists ux_mv_breadth_date
    on atlas_foundation.mv_breadth_nifty500_daily (date);

-- ── Latest snapshot per instrument (screeners / tables) ───────────────────
drop materialized view if exists atlas_foundation.mv_instrument_latest;
create materialized view atlas_foundation.mv_instrument_latest as
...
create unique index if not exists ux_mv_latest_iid
    on atlas_foundation.mv_instrument_latest (instrument_id);
