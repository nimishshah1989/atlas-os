...
def _stock_composite(dt) -> dict:
    ...
    th = _th()
    sess = _db.scalar(f"SELECT max(date) FROM {L} WHERE asset_class='stock' AND date<=:d", {"d": dt})
    df = _db.read_df(f"SELECT instrument_id, {','.join(SUBS)} FROM {L} "
                     "WHERE asset_class='stock' AND date=:d", {"d": sess})

    def f(v):
        return float(v) if v is not None and pd.notna(v) else None
    out = {}
    for _, r in df.iterrows():
        out[str(r["instrument_id"])] = float(compute_composite(
...
def _fwd_nav(mids, d0, h_days) -> dict:
    ...
    return dict(zip(nav["mstar_id"], nav["ret"]))


def run() -> None:
    snaps = [d.date() if hasattr(d, "date") else d for d in _db.read_df(
        "SELECT DISTINCT as_of_date FROM public.de_mf_holdings ORDER BY as_of_date")["as_of_date"]]
    eq = set(_db.read_df("SELECT mstar_id FROM public.de_mf_master "
                         "WHERE broad_category='Equity' AND is_active")["mstar_id"])
    print(f"snapshots={snaps}; equity funds={len(eq)}")
    for hd in (30, 60, 90):
        ics = []
        for snap in snaps:
            sess, scomp = _stock_composite(snap)
            h = _db.read_df("""SELECT mstar_id, instrument_id, weight_pct FROM public.de_mf_holdings
        ...
        print(f"  ~{hd}d fwd NAV: fund IC = {mean:+.4f} (over {len(ics)} snapshots, "
