...
CATEGORY_RULES = [
    (
        re.compile(r"large\s*(&|and)\s*mid", re.I),
        "Large & Mid Cap",
    ...
    (
        ...
        "Multi Cap",
        lambda l, m, s, e: l >= 25 and m >= 25 and s >= 25,
    ),
...
def sebi_ranks(conn):
    ...
    return {
        isin: ("large" if rk <= 100 else "mid" if rk <= 250 else "small")
...
def classify_fund(conn, mstar_id, ranks):
        ...
        """select isin, sum(weight_pct) from atlas_foundation.de_mf_holdings h
           where mstar_id = %s
             and as_of_date = (select max(as_of_date) from atlas_foundation.de_mf_holdings
                               where mstar_id = %s)
             and isin is not null group by 1""",
        (mstar_id, mstar_id),
    )
    L = M = S = U = 0.0
    ...
    return dict(
        ...
        mid_pct=round(M, 2),
        small_pct=round(S, 2),
        unclassified_pct=round(U, 2),
        equity_pct=round(L + M + S, 2),
    )
        ...
        if c["equity_pct"] + c["unclassified_pct"] > 0:
            ...
            for rx, label, rule in CATEGORY_RULES:
                if rx.search(name):
                    ...
                    detail = (
                        f"label {label}: large {c['large_pct']}% mid {c['mid_pct']}% "
