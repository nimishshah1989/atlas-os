...
def load_rates(cur):
    ...
    m = dict(cur.fetchall())
    return Decimal(str(m["portfolio_tax_ltcg_exemption_inr"])), Decimal(
        str(m["portfolio_tax_ltcg_pct"])
    )
...
def compute_client(conn, cid):
    ...
    cur.execute(
        """select coalesce(sum(l.realized_gain),0) from wealth.lots l
    ...
    cur.execute(
        """select l.lot_id, l.fund_name, l.buy_date, l.units, l.unit_basis, l.nav_now,
                          l.unrealized_gain
                   ...
                   order by l.buy_date nulls first""",
        (cid,),
    )
    gains, used = [], Decimal(0)
    for lot_id, fn, bd, units, _basis, _nav, ug in cur.fetchall():
        if used >= headroom:
            break
    ...
    cur.execute(
        """select l.lot_id, l.fund_name, l.unrealized_gain from wealth.lots l
                   join wealth.schemes s using (scheme_id)
                   where l.client_id=%s and l.status='open' and s.asset_class='Equity'
                     and l.unrealized_gain < -5000 order by l.unrealized_gain limit 10""",
        (cid,),
    )
    losses = [dict(lot_id=i, fund=f[:60], loss=round(float(g))) for i, f, g in cur.fetchall()]
    loss_note = (
        "actionable: realized gains exceed the exemption — set-off saves 12.5%"
        if realized > exempt
        else "hold: gains are within the tax-free band; harvesting losses now only "
        "creates a carry-forward (8 years, needs on-time ITR)"
    )
    # carry-forward: net realized losses by FY (informational)
    cur.execute(
        """select coalesce(sum(realized_gain),0) from wealth.lots
    ...
    return dict(
        fy=indian_fy_label(fy0),
        realized_ltcg=round(float(realized)),
        headroom=round(float(headroom)),
        gain_candidates=gains,
        gain_value=round(float(used)),
        tax_saved_if_harvested=round(float(used * ltcg_rate)),
        loss_candidates=losses,
        loss_note=loss_note,
...
def main() -> int:
    ...
    for cid in cids:
        ...
        rows.append(
            (
                ...
                r["realized_ltcg"],
                r["headroom"],
                Json(r["gain_candidates"]),
                r["gain_value"],
                r["tax_saved_if_harvested"],
                Json(r["loss_candidates"]),
                r["loss_note"],
                r["carry_forward"],
            )
        )
    ...
    print(
        f"tax harvest: {len(rows)} clients, cohort tax-saved-if-harvested ₹{tot / 1e5:.1f}L this FY"
    )
    return 0
