...
def main() -> None:
    ...
    nav = pd.read_sql(
        ...
        f"where mstar_id in {ids} and nav_date >= '2022-07-01' and nav > 0",
        conn,
    )
    nav["nav_date"] = pd.to_datetime(nav.nav_date)
    wide = (
        nav.pivot_table(index="nav_date", columns="mstar_id", values="nav").resample("W-FRI").last()
    )
    ...
    h["closet"] = (h.r2 >= 0.93) & (h.expense_ratio >= 0.8) & (h.asset_class == "Equity")
        ...
        return pd.Series(
            {
                ...
                if d.ret3y.notna().any()
                else np.nan,
                "fee_save": d.fee_save.sum(),
                "closet_mv": d.loc[d.closet, "market_value"].sum(),
                "lag_mv": lag_mv,
                "scored_mv": d.loc[d.composite.notna(), "market_value"].sum(),
            }
        )
    ...
    cat_best = (
        h.dropna(subset=["ret3y", "ann_vol", "category"])
        .assign(sharpe=lambda d: d.ret3y / d.ann_vol)
        .groupby("category")
        .agg(best_ret=("ret3y", "max"), best_sharpe=("sharpe", "max"))
    )
    hh = h.merge(cat_best, on="category", how="left")
    hh["is_lag"] = hh.cat_rank / hh.cat_size > 0.75
    swap = (
        hh[hh.is_lag & hh.best_ret.notna() & hh.ret3y.notna()]
    ...
    sec = pd.read_sql(
        ...
        where x.bucket like '%%stock%%' group by 1,2""",
        conn,
    )
    ...
    sector_cohort = (
        sec.groupby("sector")
        .agg(
            cr=("exp", lambda s: round(s.sum() / 1e7, 1)),
            clients=("client_id", "nunique"),
            med_share=("share", "median"),
        )
        .reset_index()
    ...
    for r in cl.itertuples():
        ...
        if r.xirr is not None and r.xirr < 12:
            ...
            rec_share = grp.loc[
                [d >= date(2024, 7, 1) for d in grp.inv_since], "market_value"
            ].sum() / max(grp.market_value.sum(), 1)
            if rec_share >= 0.5:
                recent_guard = True
                add(
                    r.client_id,
                    "R0 recent-buyer guard",
                    f"{rec_share:.0%} of book bought since Jul-24; XIRR {r.xirr}%",
                    "HOLD — do not churn; review at 18-month mark",
                    None,
                    "protection rule",
                )
        if lag >= 50 and not recent_guard:
            ...
            add(
                ...
                round(g),
                "₹/yr if laggards had matched best held same-category 3y CAGR (historical, not promised)",
            )
            ...
            add(
                ...
                "1-2 swaps at next review; redirect SIPs first",
                round(g),
                "same basis as R1",
            )
        ...
        if r.top_amc_share and r.top_amc_share > 0.35:
            add(
                ...
                "diversify AMC exposure on next inflows",
                None,
                "risk reduction",
            )
        if r.dup_cats and r.dup_cats >= 2:
            add(
                r.client_id,
                "R5 category duplication",
                f"{int(r.dup_cats)} sub-categories held 3+ times",
                "consolidate to best 1-2 per category",
                None,
        ...
        if r.side_pockets and r.side_pockets > 0:
            add(
                r.client_id,
                "R6 dead side-pockets",
                f"{int(r.side_pockets)} segregated-portfolio lines",
                "write-off recognition / exit paperwork",
                None,
                "hygiene",
            )
        if (
            (r.mv_hybrid / max(r.mv_total, 1)) >= 0.40
            and r.xirr is not None
            and r.xirr < 12
            and not recent_guard
        ):
            add(
                r.client_id,
                "R7 hybrid-parked underperformer",
                f"{r.mv_hybrid / r.mv_total:.0%} in hybrids; XIRR {r.xirr}%",
                "risk-capacity conversation → staged mix shift",
                None,
        ...
        if r.top_sector_share and r.top_sector_share > 0.35:
            add(
                r.client_id,
                "R8 sector concentration >35%",
                f"{r.top_sector}: {r.top_sector_share:.0%} of stock exposure",
                "trim via fund choice on next moves",
                None,
                "risk reduction",
            )
    ...
    ev(
        cur,
        "insert into wealth.client_flags (client_id, rule, evidence, action, est_value, basis) values %s",
        [
            (x["client_id"], x["rule"], x["evidence"], x["action"], x["est_value"], x["basis"])
            for x in rules
        ],
    )
    conn.commit()
    ...
    for r in cl.itertuples():
        clients_json.append(
            dict(
                ...
                name=r.full_name,
                grp=r.family_group,
                mv=jr(r.mv_total / 1e5, 1),
                xirr=jr(r.xirr),
                grade=r.outcome_grade,
                bench=jr(r.bench_xirr),
                alpha=jr(r.alpha),
                approx=bool(r.approx_flows),
                vol=jr((r.port_vol or np.nan) * 100, 1),
                volcov=jr((r.vol_cov or 0) * 100, 0),
    ...
    rule_summary = (
        fl.groupby("rule")
        .agg(clients=("client_id", "nunique"), total_value=("est_value", "sum"))
        .reset_index()
        .to_dict("records")
    )
    closet_funds = (
        ...
        .agg(
            mv_cr=("market_value", lambda s: round(s.sum() / 1e7, 2)),
            clients=("client_id", "nunique"),
            er=("expense_ratio", "median"),
            r2=("r2", "median"),
        )
        .reset_index()
        .sort_values("mv_cr", ascending=False)
        .to_dict("records")
    )
    ...
    out = dict(
        ...
        rules=[
            {k: (jr(v, 0) if k == "total_value" else v) for k, v in r.items()} for r in rule_summary
        ],
        flags=rules,
        closet_funds=closet_funds,
        sector_cohort=sector_cohort.assign(
        ...
        fees=dict(
            total=jr(cl.fee_save.sum(), 0),
            clients=int((cl.fee_save > 10000).sum()),
    ...
    print(
        f"clients={len(clients_json)} bench_computed={out['beat']['n']} "
    ...
    print(
        f"closet-indexer value ₹{out['fees']['closet_mv_cr']}cr, fee savings ₹{out['fees']['total']:,}/yr"
    )
