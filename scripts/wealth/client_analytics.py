...
def main() -> None:
    ...
    # ---- flows-based benchmark alpha ----
    bdates = bench_daily.index
    bvals = bench_daily.to_numpy()

    def bnav(d: date) -> float | None:
        i = bdates.searchsorted(pd.Timestamp(d))
        if i >= len(bvals):
            i = len(bvals) - 1
        return float(bvals[i])

    bench_rows = []
    for cid, grp in hold.groupby("client_id"):
        flows = [(r.inv_since, -float(r.investments)) for r in grp.itertuples()
                 if r.investments and r.investments > 0 and r.inv_since]
        if not flows:
            continue
        flows.sort()
        units = 0.0
        ...
        for d, amt in flows:
            nv = bnav(d)
            if nv is None or d < date(2006, 4, 3):
                skip = True
                break
            units += (-amt) / nv
        if skip:
            continue
        terminal = units * bnav(AS_ON)
        bx = xirr(flows + [(AS_ON, terminal)])
        if bx is not None:
            bench_rows.append((cid, bx))
    bench = pd.DataFrame(bench_rows, columns=["client_id", "bench_xirr"])

    cl = rep.merge(per, on="client_id", how="left").merge(bench, on="client_id", how="left")
    cl["approx_flows"] = (cl.gross_out + cl.dividend_payouts) > 0.10 * cl.gross_in
