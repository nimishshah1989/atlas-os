"""v6 — mv_india_pulse v2: macro date-tolerant lateral join + DMA->EMA breadth
labels + 20/100 EMA + 4-week-high breadth rows + 5-window sector RS heatmap.

Regime + India Pulse fix chunk (2026-05-30).

Root causes fixed (all live-verified on Supabase atlas-os):
  (A) Term structure / macro cards / narrative ribbon were BLANK because the
      MV joined atlas_macro_daily ON date = as_of_date (exact). Macro lags the
      regime date (e.g. macro 05-27 vs regime 05-29) so the newest MV row got
      NULL macro. Fixed with date-tolerant LATERAL "latest macro row <= as_of".
  (B) Breadth rows were mislabelled "% above NNN DMA" though the data is EMA.
      Relabelled to "% above NNN EMA"; added "% above 20 EMA", "% above 100 EMA"
      and "% at 4-week high" rows. The two new metrics read new nullable columns
      pct_above_ema_100 + pct_4w_high (added here); data_gap auto-flips false
      once the breadth backfill populates them. No second migration needed.
  (C) Sector heatmap emitted rs_1w only; now emits rs_1w/1m/3m/6m/12m.

Forward-only in spirit but reversible: downgrade restores the migration-100 body.
"""

# allow-large: dual MV body (v2 + migration-100 restore) for a reversible downgrade
...
ALTER TABLE atlas.atlas_market_regime_daily
  ADD COLUMN IF NOT EXISTS pct_above_ema_100 numeric,
  ADD COLUMN IF NOT EXISTS pct_4w_high numeric;
"""

_DROP_COLS = """
ALTER TABLE atlas.atlas_market_regime_daily
  DROP COLUMN IF EXISTS pct_above_ema_100,
  DROP COLUMN IF EXISTS pct_4w_high;
"""
...
_REFRESH = "REFRESH MATERIALIZED VIEW atlas.mv_india_pulse;"
...
def upgrade() -> None:
    """Add breadth columns, recreate mv_india_pulse (v2 body), reindex + refresh."""
    op.execute(_ADD_COLS)
    op.execute(_DROP_MV)
    op.execute(_CREATE_MV_V2)
    op.execute(_UNIQUE_INDEX)
    op.execute(_REFRESH)


def downgrade() -> None:
    """Restore the migration-100 MV body; drop the two added columns last."""
    op.execute(_DROP_MV)
    op.execute(_CREATE_MV_OLD)
    op.execute(_UNIQUE_INDEX)
    op.execute(_REFRESH)
    op.execute(_DROP_COLS)
