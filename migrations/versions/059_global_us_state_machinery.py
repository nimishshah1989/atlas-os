...
def upgrade() -> None:
    ...
    op.execute("""
        INSERT INTO us_atlas.atlas_thresholds
            (key, value, category, description, unit, min_value, max_value, default_value)
        VALUES
            ('rs_quintile_top',                  0.80, 'rs',     'RS pctile above which = Leader/Strong',     'ratio',   0.60, 0.95, 0.80),
            ('rs_quintile_bottom',               0.20, 'rs',     'RS pctile below which = Laggard/Weak',      'ratio',   0.05, 0.40, 0.20),
            ('momentum_flat_band_pct',           0.02, 'momentum','EMA10 within ±2% of 1.0 = Flat',          'ratio',   0.01, 0.05, 0.02),
            ('momentum_ema_convergence_pct',     0.01, 'momentum','EMA10-EMA20 within 1% = converged',        'ratio',   0.005,0.03, 0.01),
            ('risk_extension_low_max_pct',       25.0, 'risk',   'Extension below 25% = Low risk',            'pct',     10.0, 40.0, 25.0),
            ('risk_extension_high_min_pct',      40.0, 'risk',   'Extension above 40% = High risk',           'pct',     25.0, 60.0, 40.0),
            ('risk_vol_ratio_low_max',            1.0, 'risk',   'Vol ratio below 1.0 = Low volatility',      'ratio',   0.5,  1.5,  1.0),
            ('risk_vol_ratio_normal_max',         1.25,'risk',   'Vol ratio below 1.25 = Normal volatility',  'ratio',   1.0,  1.75, 1.25),
            ('risk_vol_ratio_high_min',           1.6, 'risk',   'Vol ratio above 1.6 = High volatility',     'ratio',   1.25, 2.5,  1.6),
            ('volume_accumulation_expansion_min', 1.2, 'volume', 'Vol expansion ≥1.2 = Accumulation',         'ratio',   1.0,  1.5,  1.2),
            ('volume_accumulation_effort_min',    1.3, 'volume', 'Effort ratio ≥1.3 = Accumulation',          'ratio',   1.0,  1.8,  1.3),
            ('volume_distribution_effort_max',    0.8, 'volume', 'Effort ratio ≤0.8 = Distribution',          'ratio',   0.5,  1.0,  0.8),
            ('volume_heavy_distribution_effort_max',0.6,'volume','Effort ratio ≤0.6 = Heavy Distribution',    'ratio',   0.3,  0.8,  0.6)
    ...
    op.execute("""
        ...
        VALUES
            ...
            ('volume_distribution_effort_max',    0.8, 'volume', 'Effort ratio ≤0.8 = Distribution',          'ratio',   0.5,  1.0,  0.8),
            ('volume_heavy_distribution_effort_max',0.6,'volume','Effort ratio ≤0.6 = Heavy Distribution',    'ratio',   0.3,  0.8,  0.6),
            ('liquidity_gate_min_avg_vol',      10000, 'gate',   'Min 20-day avg volume (shares)',             'shares',  1000,100000,10000)
        ON CONFLICT (key) DO NOTHING
    """)
