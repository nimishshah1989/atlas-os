...
def upgrade() -> None:
    ...
    op.execute(sa.text("""
        INSERT INTO atlas.atlas_thresholds (threshold_key, value, description)
        VALUES
            ('holdings_weight_change_min_pct', 0.25, 'Min absolute weight delta to classify as increase/decrease'),
            ('decision_score_sharp_threshold', 65.0, 'Signal score >= this => Sharp decision state'),
            ('decision_score_poor_threshold',  40.0, 'Signal score < this => Poor decision state')
