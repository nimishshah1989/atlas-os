...
- **2026-07-02 (session 2c) — RENAME → atlas_foundation + CRON cleaned + dead-code purge (FM directive).**
  ...
  - **Dead-code purge IN PROGRESS:** backend import-reachability (roots = the live orchestrator entrypoints) found **55 reachable / 253 dead** .py under scripts+atlas (retired M1-M7 / intelligence / strategy / CTS / us-global / tv / old-FastAPI). Removed `scripts/loops/` (17 stale methodology docs). A classifier subagent is separating SAFE_DELETE (retired) from KEEP (maintenance tools that produce live tables / power /admin) before the mass git-rm. Frontend was already purged in G1 (475 files).
